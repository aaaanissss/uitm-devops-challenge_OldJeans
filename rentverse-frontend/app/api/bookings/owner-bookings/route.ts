...
export async function GET(request: NextRequest) {
    try {
        ...
        const endpoint = queryString ? `api/bookings/owner-bookings?${queryString}` : 'api/bookings/owner-bookings'
        ...
        try {
            ...
            console.error('Backend error during owner-bookings:', backendError)
            return NextResponse.json(
        ...
        return NextResponse.json(
            createErrorResponse('Failed to get owner bookings', error as Error),
            { status: 500 },
        )
    }
}
