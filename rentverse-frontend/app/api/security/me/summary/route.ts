...
export async function GET(request: NextRequest) {
    try {
        ...
        try {
            // Forward to backend
            const response = await forwardRequest('api/security/me/summary', {
                method: 'GET',
                headers: {
                    ...authHeader,
            ...
            console.error('Backend error during security summary:', backendError)
            return NextResponse.json(
        ...
        return NextResponse.json(
            createErrorResponse('Failed to get security summary', error as Error),
            { status: 500 },
        )
    }
}
