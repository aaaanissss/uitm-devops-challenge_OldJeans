...
export async function GET(request: NextRequest) {
    try {
        ...
        const endpoint = queryString ? `api/security/audit-logs/summary?${queryString}` : 'api/security/audit-logs/summary'
        ...
        try {
            ...
            console.error('Backend error during audit-logs summary:', backendError)
            return NextResponse.json(
        ...
        return NextResponse.json(
            createErrorResponse('Failed to fetch audit logs summary', error as Error),
            { status: 500 },
        )
    }
}
