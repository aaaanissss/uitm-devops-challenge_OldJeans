...
export async function GET(request: NextRequest) {
    try {
        ...
        try {
            ...
            console.error('Backend error during properties list:', backendError)
            return NextResponse.json(
        ...
        return NextResponse.json(
            createErrorResponse('Failed to list properties', error as Error),
            { status: 500 },
        )
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            ...
            console.error('Backend error during property creation:', backendError)
            return NextResponse.json(
                createErrorResponse('Backend service unavailable', backendError as Error, 503),
                { status: 503 }
            )
